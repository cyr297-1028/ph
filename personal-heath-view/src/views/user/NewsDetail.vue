    ...
    <el-row>
        ...
        <el-col :span="6">
            <div v-if="scoreList.length === 0">
                <h3 style="padding: 10px 30px;">立即评分</h3>
                <div style="background-color: rgb(248,249,250);margin-inline: 30px;padding: 20px;border-radius: 4px;">
                    <el-rate style="margin-left: 25px;" v-model="score" @change="scoreChange"></el-rate>
                </div>
            </div>
            <div v-else>
                <h3 style="padding: 10px 30px;">我的评分</h3>
                <div style="background-color: rgb(248,249,250);margin-inline: 30px;padding: 20px;border-radius: 4px;">
                    <el-rate style="margin-left: 25px;" v-model="scoreList[0].total" disabled show-score
            ...
            <el-col @click.native="newsItemClick(news)" :span="24" :key="index" v-for="(news, index) in newsTopList">
                <div style="padding: 8px 30px;box-sizing: border-box;">
                    <img :src="news.cover" :alt="news.name" style="width: 100%;height: 148px;border-radius: 5px;">
...
export default {
    ...
    methods: {
        scoreChange() {
            const dto = {
                total: this.score,
                articleId: this.newsInfo.id
            }
            this.$axios.post('/score/save', dto).then(response => {
                const { data } = response;
                if (data.code === 200) {
                    this.$notify({
                        duration: 1000,
        ...
        loadScore() {
            const dto = { articleId: this.newsInfo.id };
            this.$axios.post('/score/query', dto).then(response => {
                const { data } = response;
                if (data.code === 200) {
        ...
        // 查询推荐资讯
        loadAllTopNews() {
            this.$axios.get(`/news/queryRecommend/${3}`).then(response => {
                const { data } = response;
                if (data.code === 200) {
